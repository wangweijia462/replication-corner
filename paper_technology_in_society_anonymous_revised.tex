...
\end{tabular}}
\begin{minipage}{0.96\textwidth}
\footnotesize Notes: The dependent variable is the 20 percent patent-quality index. Columns 1--4 use the full sample with the institutional index. Columns 5 and 6 split the sample by above- and below-median institutional quality and report conditional certification effects; these split-sample coefficients are descriptive complements and should not be read as formal tests of the interaction hypothesis. Firm controls include log employment, tangibility, ROA, ROE, cash holdings, leverage, sales growth, and earnings-to-assets. Standard errors are in parentheses and clustered as indicated. A supplementary wild-cluster bootstrap test for the preferred interaction specification, using province clusters, Webb weights, and 9,999 replications, gives \(p=0.280\); the corresponding Rademacher-weight result is similar at \(p=0.274\). * \(p<0.10\), ** \(p<0.05\), *** \(p<0.01\).
\end{minipage}
\end{table}

\subsection{Event-study evidence and identification}

Figure 1 reports the event-study estimates from the replication specification, normalized to the year before certification. The post-certification coefficients are positive in all seven post-treatment periods, ranging from about 0.045 to 0.104. The displayed pre-treatment coefficients are also positive but imprecisely estimated, and their confidence intervals include zero. This pattern is more consistent with parallel pre-treatment dynamics than with a clear anticipatory response.

The event-study evidence is reassuring, but it does not by itself make certification random. A conventional DID interpretation still requires treated and comparison firms to follow parallel counterfactual trends after conditioning on firm and year fixed effects and controls. The SRDI program's design makes that assumption demanding because official criteria emphasize accumulated technological capability, specialization, R\&D intensity, and intellectual property ownership. The paper therefore treats the estimates as quasi-experimental evidence rather than as a clean experimental contrast. A direct test of remaining selection would compare certified firms with rejected applicants or eligible nonrecipients using application records that are not available in the current submission.
...
\begin{minipage}{0.96\textwidth}
\footnotesize Notes: Relative year -1 is normalized to zero. Displayed pre-period indicators cover years -5 through -2; earlier years are excluded because fewer than 10 percent of treated firms have observations before year -5, making those estimates unreliable. Displayed post-period indicators cover years 0 through +6, with years \(\geq +6\) binned. The specification includes firm controls, firm fixed effects, and year fixed effects. Standard errors are clustered at the province level. The horizontal axis shows years relative to SRDI certification; t\,=\,0 is the first year of certification.
\end{minipage}
\end{figure}

\subsection{Endogeneity and robustness checks}

Table 3 reports matching and selection checks addressing nonrandom certification timing. The interaction coefficient remains positive in the PSM-RD, PSM-size, and Heckman specifications. These estimates do not solve the selection problem, but they show that the positive interaction pattern is not limited to the baseline regression.
...
\caption{Distribution of 200 permuted interaction coefficients (placebo test)}
\begin{minipage}{0.96\textwidth}
\footnotesize Notes: In each iteration, institutional quality is randomly permuted within calendar year and the baseline interaction model is re-estimated. The histogram and kernel density overlay show the full distribution of the 200 permuted interaction coefficients. The dashed vertical line marks the observed interaction estimate of 0.046. None of the 200 permuted estimates exceeds the observed estimate, giving a finite-simulation permutation \(p\)-value of 0.004975 ($= 1/200$). This test evaluates whether the observed interaction is unusually large relative to randomized institutional assignments; it does not replace cluster-robust inference for sampling uncertainty.
\end{minipage}
\end{figure}
...
The study has five important limitations. First, SRDI certification is not randomly assigned. While event-study pre-trends are imprecise and statistically indistinguishable from zero---a pattern consistent with parallel pre-treatment dynamics---the SRDI program's selection criteria mean that treated firms may differ from controls in unobserved capability trajectories that the event-study window cannot fully rule out. Second, the province-level number of clusters limits statistical power. Several estimates are directionally stable but imprecise, and wild-cluster bootstrap inference confirms that the baseline interaction is not statistically strong. Third, the IV specification has weak-instrument concerns, so it cannot solve endogeneity on its own (see Appendix Table A1). Fourth, the sample covers listed firms, which are more visible and better resourced than the broader population of SMEs. Fifth, the paper tests several mechanism outcomes and subgroups; the mechanism evidence should therefore be interpreted with multiple-testing caution.
...
Hu, A. G. Z., Zhang, P., and Zhao, L. (2017).
China as number one? Evidence from China's most recent patenting surge.
\textit{Journal of Development Economics}, 124, 107--119.
...
The fostering policy for ``Little Giants'' and innovation quality: Empirical evidence from invention patents of listed enterprises.
\textit{The Journal of World Economy}, 49(4), 36--67. [In Chinese].

\bibitem[Lin et al.(2021)]{lin2021}
Lin, J., Wu, H.-M., and Wu, H. (2021).
Could government lead the way? Evaluation of China's patent subsidy policy on patent quality.
\textit{China Economic Review}, 69, Article 101663.
