...
We conceptually replicate this finding using 843 peer-reviewed
Chinese business and management journal articles indexed on CNKI
(2023--2026).
AI-like writing style is proxied by a dictionary score capturing the
prevalence of 23 pre-specified AI-associated structural markers in
abstract text; institutional prestige is coded via a four-tier Chinese
university designation scheme (details in Section~\ref{sec:method}).
...
affiliation in peer-reviewed venues---a pattern absent from the SSRN
preprint context.
The Prestige\,$\times$\,AI-style interaction is near zero and
non-significant across all four specifications.
These results identify a boundary condition for the mechanism documented
by \citet{JoLi2026}: AI can attenuate reputation-based diffusion only
where a prestige premium is already operative.
...
We initially collected 856 CNKI records; records outside the 2023--2026
window, with incomplete abstracts, or lacking formal publication
identifiers were excluded, yielding a final analytic sample of
\textbf{843 articles}.
All articles come from CNKI journals in business-relevant categories
(economics, management, and business administration), mirroring the
marketing and business-adjacent knowledge diffusion context of
\citet{JoLi2026}'s SSRN sample.
Topic domains include marketing strategy and consumer behavior,
AI and digital transformation, supply chain and operations, corporate
governance, and ESG.
A marketing and management sub-sample robustness check is reported in
Web Appendix~B7.
...
We interpret this evidence as covering business- and
marketing-adjacent knowledge diffusion rather than a general
bibliometric analysis of AI scholarship.
...
23~pre-specified AI writing-style markers in the abstract text and
dividing the matched-marker count by 23, yielding a $[0,1]$
score.\footnote{The complete marker list with Chinese original text,
  English translations, and per-marker frequencies is provided in
  Web Appendix~A2.}
Scoring is applied to abstract text as retrieved from CNKI, which
truncates displayed abstracts at approximately 203 characters;
markers appearing only in the later portions of longer abstracts may
be under-counted, a limitation discussed in
...
\subsection[RQ1: Prestige x AI-Style Interaction]{RQ1: The Prestige\,$\times$\,AI-Style Interaction}
...
These findings caution against generalizing AI's equalizing role
across publication platforms; future research with direct AI-use
records and multiple platforms is needed to establish the scope of
the effect.
...
systematically under-represents AI-like writing patterns in the most
recent articles, potentially attenuating estimated effects toward zero.
...
false negatives (AI-assisted authors who avoid them) introduce
attenuation bias in opposite directions; accordingly, the positive
download main effect should be interpreted as suggestive rather
than definitive.
...
\newpage
% ---- Web Appendix Reference -------------------------------------------------
\section*{Web Appendix}

The complete Web Appendix is submitted as a separate document
(\textit{web\_appendix.pdf}).
It contains:
\textbf{Appendix~A} (Data Construction): CNKI search string and sample
flow (A1); full 23-marker dictionary with Chinese text, English
translations, and per-marker frequencies, plus AI-style score trend
by year (A2); prestige tier distribution and first-author prestige
sensitivity analysis (A3); variable dictionary (A4).
\textbf{Appendix~B} (Additional Analyses): full regression coefficient
tables including intercepts and year fixed effects (B1); complete
robustness specification tables (B2); negative-binomial models on raw
counts with incidence rate ratios (B5); topic fixed-effect sensitivity
(B6); marketing/management sub-sample robustness, $n=239$ (B7).
\textbf{Appendix~C}: side-by-side comparison with Jo and Li (2026).
