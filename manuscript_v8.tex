...
AI involvement is measured via an abstract-level dictionary-based score
that counts the density of 23 pre-specified AI-associated structural
writing markers; institutional prestige is coded on a four-tier
classification based on China's C9, 985, and 211/Double First-Class
university designations.
AI-style writing independently predicts higher log-transformed download
counts ($\hat{\beta}=0.22$, $p<.001$), partially replicating the main
effect documented by \citet{JoLi2026}.
However, the institutional prestige main effect is not statistically
significant across any specification ($\hat{\beta}=0.11$, $p=.10$
in the primary download model; all $p>.10$), suggesting that peer-reviewed
editorial gatekeeping may itself neutralize the prestige premium in this
context---a pattern not observed in the SSRN preprint ecosystem.
The Prestige\,$\times$\,AI-style interaction is consistently
...
These results suggest that the prestige premium itself---the precondition
for the moderation mechanism documented by \citet{JoLi2026}---is absent
in this peer-reviewed Chinese context, constituting a more fundamental
boundary condition than originally hypothesized and raising new questions
about the conditions under which institutional prestige shapes knowledge
diffusion.
...
AI involvement is proxied by an abstract-level dictionary score that
...
The null prestige main effect is itself theoretically informative:
it suggests that peer-reviewed editorial certification may already
neutralize the prestige premium in formal publication venues, making
the moderation question moot in this context.
...
patterns of LLM-assisted academic writing, and \citet{Biber2016}
characterises the broader structural features of academic English style.
Although \citet{Biber2016} analyze English-language corpora, the
structural properties they document---formulaic section organization,
templated summary phrases, and stereotyped transitional markers---manifest
analogously in Chinese LLM-assisted academic abstracts, providing
cross-linguistic grounding for our marker taxonomy.
...
The \textit{ai\_style\_score} is an abstract-level measure of AI-associated
writing style, constructed by scoring the binary presence of each of
23~pre-specified AI writing-style markers in the Chinese abstract text
and dividing the count of matched markers by 23, yielding a score
$\in[0,1]$ that reflects the structural-formulaic density of
AI-associated patterns.\footnote{The complete marker list with Chinese
  original text, English translation, and example abstracts is provided
  in Web Appendix~A2.}
...
of 2023--2024 abstracts independently classified as AI-assisted
or non-AI-assisted by two bilingual coders; discrepancies were resolved
by discussion, yielding a final dictionary of structured section-markers
(e.g., \textit{yanjiu faxian} [`results show'],
\textit{jizhi fenxi} [`mechanism analysis'],
\textit{yizhixing fenxi} [`heterogeneity analysis'],
\textit{wenjianxing jianyan} [`robustness checks'])
and formulaic template phrases.
Specifically, the two coders independently applied all candidate marker
criteria to the development abstracts to resolve boundary definitions
(e.g., whether a phrase must appear verbatim or allows paraphrase),
and their agreement on marker applicability was assessed before
finalizing the dictionary.
The dictionary-construction process yielded inter-coder agreement of
Cohen\textquotesingle s $\kappa = 0.79$ on the development sample,
exceeding the conventional 0.70 threshold for acceptable agreement
and indicating that the marker boundary definitions were applied
consistently by both annotators.
Second, for each target abstract, we assessed the binary presence of
each of the 23 markers via exact string matching; the score equals
the proportion of markers present.
This transparent, fully deterministic scoring procedure requires no
external API and is entirely reproducible from the published marker
list.
The resulting score ranges from 0 to 0.261
(mean\,=\,0.049, median\,=\,0.044, SD\,=\,0.050).
...
construct validity limitation addressed further in §\ref{sec:discussion}.
...
of 843) had received zero citations, reflecting that 23\% of articles
(those published in 2023) have had more than two years of citation
exposure while 2026 articles are very recently published.
The citation-based results should nonetheless be interpreted with
appropriate caution given the range of publication ages.
...
The null prestige main effect is itself substantively informative
and is discussed in the context of boundary conditions in
§\ref{sec:discussion}.
...
This certification may attenuate the marginal informativeness of
institutional affiliation for readers: once an article has passed
peer review, its institutional pedigree contributes little additional
quality information.
If certification effectively equalizes the prestige premium, the AI-style
moderation mechanism becomes irrelevant by construction---which is
precisely the pattern we observe.

The structure of China's academic system may further contribute to
this dynamic.
Although prestige classifications (C9, 985, 211) are formally
institutionalized, the sheer diversity of journals indexed on CNKI
and the breadth of topics in our sample---spanning AI, ESG, supply
chain, governance, and organizational management---may dilute any
single prestige signal relative to content-area relevance.
We note, however, that the null interaction is also consistent with
alternative explanations, including measurement error in the AI-style
proxy that attenuates the estimated interaction toward zero, and
insufficient statistical power given the sample size of 843;
the present data do not allow us to empirically distinguish between
these accounts.
...
Indeed, our results suggest a more nuanced picture: in peer-reviewed
Chinese journals, the prestige premium appears attenuated even without
AI assistance, implying that editorial gatekeeping may itself be the
primary equalizer rather than AI-style writing.
...
contexts.
Tentatively, if AI democratization is to materialize in journal publishing,
the relevant mechanism may differ from the stylistic-homogenization
channel: changes to peer review composition, editorial networks, and
funding evaluation criteria may matter more; however, given the
observational nature of this single-context study, these implications
should be treated as questions for future research rather than
confirmed policy recommendations.

\subsection{Limitations}

Several limitations qualify these conclusions.
First, and most critically, the \textit{ai\_style\_score} poses a
\textit{construct validity} threat: as a dictionary-based measure
of structural writing patterns in abstract text,
it captures AI-\textit{like} writing style rather than confirmed
AI tool use.
...
Seventh, using a simple keyword-matching dictionary to score abstracts
for AI-writing style may miss contextual and syntactic AI-associated
features that only a language model can detect; future work should
validate the dictionary-based score against GPT-based evaluation or
human annotation.
...
peer-reviewed journal context using 843 CNKI
(China National Knowledge Infrastructure) articles.
The AI-style writing main effect replicates for downloads: formulaic
AI-associated writing patterns predict greater download volume.
However, the institutional prestige main effect does not replicate---articles
from high-prestige institutions show no significant download or citation
advantage---and the Prestige\,$\times$\,AI interaction is consistently
near zero and statistically insignificant across all specifications.
Peer-reviewed journal gatekeeping and the institutionalized prestige
hierarchy of the Chinese academic system appear to suppress the
reputation premium that drives the moderation mechanism in the SSRN
preprint context, at least within Chinese business and management scholarship.
Future research should test whether this context-specific pattern holds
across other formal publication platforms, disciplines, and national
contexts using larger samples, and should investigate whether direct
measures of AI tool use---rather than stylometric proxies---reveal
...
\item \textit{Note.} Prestige\_High\,=\,1 if any author is affiliated with a
  ...
  at the truncation maximum, producing a left-skewed distribution with a
  small standard deviation despite a non-trivial minimum.
...
\item \textit{Note.} OLS with HC3 heteroskedasticity-robust standard errors
  ...
  $^{*}p<.05$;\ $^{**}p<.01$;\ $^{***}p<.001$.
  Columns~1--2 follow Equation~(\ref{eq:binary}) (binary prestige);
  Columns~3--4 follow Equation~(\ref{eq:score}) (standardized
  prestige score); both equations are estimated separately for
