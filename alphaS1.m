...
% parameters 
global alphaM alphaP epsilon rho ksiN0 ksiN1 ksiE0 ksiE1 zetaN zetaE sigma etaN etaE e_bar betaM betaP alphaMS alphaPS zetaND zetaED
...
alphaMS    = 0.5;   % sensitivity
alphaPS    = 0.5;   % sensitivity
zetaND     = 0.5;   % sensitivity
zetaED     = 0.5;   % sensitivity
...
% sensitivity
alphaMS    = 0.4;   % sensitivity
alphaPS    = 0.4;   % sensitivity
...
% equilibrium
    ...
    pN          = (zetaND*AN^(sigma-1) + zetaN*kappaN^(sigma-1)*WN_tilde^(1-sigma))^(1/(1-sigma));
    pE          = (zetaED*AE^(sigma-1) + zetaE*kappaE^(sigma-1)*WE_tilde^(1-sigma))^(1/(1-sigma));
    n           = (etaN-etaE)/(1+etaN) * (W0+W1)/(pN-pE*e_bar);
    e_prime     = etaE/(etaN-etaE)*pN/pE - etaN/(etaN-etaE)*e_bar;
    WNlN_tilde  = zetaN*kappaN^(sigma-1)*WN_tilde^(1-sigma) / (zetaN*kappaN^(sigma-1)*WN_tilde^(1-sigma) + zetaND*AN^(sigma-1))*pN*n;
    WElE_tilde  = zetaE*kappaE^(sigma-1)*WE_tilde^(1-sigma) / (zetaE*kappaE^(sigma-1)*WE_tilde^(1-sigma) + zetaED*AE^(sigma-1))*pE*e_prime*n;
    ...
    wP          = (W1 - W0)/(1-chi);
    PM_tilde    = (alphaMS*BM^(epsilon-1) + alphaM*wM^(1-epsilon))^(1/(1-epsilon));
    PP_tilde    = (alphaPS*BP^(epsilon-1) + alphaP*wP^(1-epsilon))^(1/(1-epsilon));
...
% output
delta_alphaS1_BM = [delta_n_BM;delta_e_prime_BM;delta_omega_BM;delta_W_BM;delta_L0_BM;delta_L1_BM]; 
delta_alphaS1_BP = [delta_n_BP;delta_e_prime_BP;delta_omega_BP;delta_W_BP;delta_L0_BP;delta_L1_BP]; 
delta_alphaS1_AE = [delta_n_AE;delta_e_prime_AE;delta_omega_AE;delta_W_AE;delta_L0_AE;delta_L1_AE]; 
delta_alphaS1_AN = [delta_n_AN;delta_e_prime_AN;delta_omega_AN;delta_W_AN;delta_L0_AN;delta_L1_AN]; 

save delta_alphaS1 delta_alphaS1_BM delta_alphaS1_BP delta_alphaS1_AN delta_alphaS1_AE;  % alphaMS = alphaPS = 0.4
