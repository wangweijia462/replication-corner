...
These results suggest that the reputation-equalizing
potential of AI-style writing documented for SSRN preprints may not
extend to peer-reviewed publication contexts where editorial gatekeeping
provides an independent quality signal---at least within the Chinese peer-reviewed business and management context.
...
specifications, suggesting that peer-reviewed journal publication
may represent a boundary condition, at least within the Chinese peer-reviewed business and management context.
...
Although \citeauthor{Biber2016} analyze English-language corpora, the
structural properties they document---formulaic section organization,
high nominal density, and templated transitional phrases---manifest
...
writing style, generated via a structured ChatGPT evaluation (model:
\texttt{gpt-4o-2024-08-06}; \texttt{temperature=0} for
deterministic, version-locked outputs) of each Chinese abstract
against a pre-specified dictionary of 23~AI-writing style markers.\footnote{Full prompt text, the complete
  marker list with Chinese original and English translations, and the
  dictionary version number (frozen prior to scoring to prevent
  post-hoc adjustment) are provided in Web Appendix~A2.}
...
Second, ChatGPT evaluated each target abstract using the dictionary
as a reference rubric, producing a 0--1 score that reflects both
the explicit density of matched markers and the overall
structural-formulaic character of the abstract text.
Separately, to assess the reliability of the ChatGPT scoring procedure
itself, a held-out validation subsample of 30~abstracts was
independently scored by a human rater blind to the ChatGPT output;
Pearson $r = 0.82$ ($p < .001$) between the two sets of scores
indicates acceptable convergent reliability between the automated
scoring pipeline and human judgment.
The resulting score ranges from 0 to 1
(mean\,=\,0.257, median\,=\,0.246, SD\,=\,0.130).
...
At the time of data collection, approximately 83\% of articles ($n = 397$
of 480) had received zero citations, reflecting the short publication window
(most articles published in 2024--2026) and the relatively modest
citation velocity on CNKI for newly-published work.
The citation-based results should therefore be treated as exploratory.
The correlation between $\log(\text{Age})$ and log downloads is
$r\,=\,.58$; the higher correlation with log citations ($r\,=\,.68$)
likely reflects that the few articles that have accumulated citations
are also the oldest in the sample, amplifying the age--citation
association relative to the age--download association.
...
coinciding with the Column~1 point estimate---the SE difference reflects
the adjustment in covariate structure under the continuous-prestige
specification).
...
$\hat{\beta}_3 = -0.041$ (SE\,=\,0.046, $p\,=\,.373$;
95\,\%\,CI: [$-$0.131, $+$0.049]).
...
In the baseline models, overall fit is strong ($R^2 = 0.430$ for
downloads, $R^2 = 0.573$ for citations), with publication age
($\hat{\beta}_{\log\text{Age}}\approx 0.97$) the dominant predictor,
confirming that cumulative exposure time---not writing style or
prestige alone---drives most of the explained variance.
...
($\hat{\beta}_3 = 0.010$) and citations ($\hat{\beta}_3 = -0.053$).
...
in all specifications; all six Prestige\,$\times$\,AI interaction
terms are non-significant (additional $p$-values and confidence
intervals are available in Web Appendix Table~B2).
...
prestige premium, suggesting that peer-reviewed journal publication
may represent a boundary condition for the effect documented on SSRN,
at least within the Chinese peer-reviewed business and management context.
...
insufficient statistical power given the sample size of 480;
the present data do not allow us to empirically distinguish between
these accounts.
The width of the 95\,\% confidence intervals (spanning approximately
$\pm$0.2 log-units for the binary-prestige interactions) indicates
that the design would be unable to reliably detect effects smaller
than roughly half a standard deviation, motivating larger-sample
replications.
...
contexts.
Tentatively, structural changes to peer review, editorial composition,
and funding evaluation criteria may be required if AI's democratizing
promise is to materialize in journal publishing; however, given the
...
\textit{construct validity} threat: as a stylometric measure generated
by evaluating abstract text with ChatGPT against a fixed dictionary,
...
Sixth, using a GPT-4o model to score abstracts for AI-writing style
introduces potential circularity: the scoring model may assign higher
scores to text that is stylistically similar to its own outputs,
independently of the pre-specified marker dictionary.
Future work should validate the scores against human annotation or
an AI-detection tool that is architecturally independent of GPT-family
models.
...
peer-reviewed journal context using 480 CNKI
...
contexts, and should investigate whether direct measures of AI tool
use---rather than stylometric proxies---reveal stronger or
...
(3) Prestige\_High & 0.59 & 0.49 & 0 & 1 & 1 &
  .21** & .28** & --- &     &     &     & \\
(4) ai\_style\_score & 0.257 & 0.130 & 0.040 & 0.246 & 0.63 &
  .18** & .15** & $-$.08 & --- &     &     & \\
...
\item \textit{Note.} Prestige\_High\,=\,1 if any author is affiliated with a
  ...
  The modest correlation between Prestige\_High and Author count
  ($r=.09$, $p=.048$) does not raise multicollinearity concerns
...
3 & Downloads & Exclude 2026 articles ($n = 383$)
  & $0.498^{***}$ & $0.241^{**}$  & $-0.023$ \\
  &           &
...
ChatGPT prompt text (model: \texttt{gpt-4o-2024-08-06},
\texttt{temperature=0}) and dictionary version number;
score distribution histogram and percentile table;
inter-coder agreement for dictionary construction ($\kappa=0.79$,
development sample) and ChatGPT--human convergent reliability
($r=0.82$, 30-article held-out subsample).
