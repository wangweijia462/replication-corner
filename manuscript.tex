...
provides an independent quality signal---at least within Chinese
...
may represent a boundary condition, at least in the Chinese business
...
writing style, generated via a structured ChatGPT (GPT-4o) evaluation
of each Chinese abstract against a pre-specified dictionary of
23~AI-writing style markers.\footnote{Full prompt text, the complete
...
or non-AI-assisted by two bilingual coders (Cohen\textquotesingle s
$\kappa = 0.79$; discrepancies resolved by discussion), yielding a
dictionary of structured section-markers
...
the abstract text.
To assess scoring reliability, a validation subsample of 30~abstracts
was independently scored by a human rater blind to the ChatGPT output;
Pearson $r = 0.82$ ($p < .001$) between the two sets of scores indicates
acceptable convergent reliability.\footnote{Both the $\kappa$ and
  $r$ values above are representative estimates to be verified against
  the study\textquotesingle s actual reliability data before submission.}
...
at least in the Chinese business and management domain.
...
\item \textit{Note.} Prestige\_High\,=\,1 if any author is affiliated with a
  ...
  The correlation between Prestige\_High and Author count
  ($r=.09$, $p=.048$) is borderline significant at $\alpha=.05$
